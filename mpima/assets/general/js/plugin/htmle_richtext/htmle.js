;(function ($, undefined) {
    ...
    function Htmle(element, options) {
	 ...
	 Htmle.prototype.content = function () {
        //console.log('gggg');
    }
    ...
    Htmle.prototype._init = function () {
		...
		if(this.options.textareaContainer)
        var htmlearea = this.$E.parents().find(this.options.textareaContainer);
		else
        {
            var htmlearea = $('<div class="htmle">');
			this.$E.after(htmlearea);
			this.options.textareaContainer='div.htmle'
		}
        ...
        if (this.options.height)
            htmleview.css('height', this.options.height);
				
        if (this.options.width)
            htmleview.css('width', this.options.width);
        ...
        htmlearea.prepend(htmleview);

        var scope = this;
        this.$E.change(function () {
            var bdy = scope.$E.parents().find(scope.options.textareaContainer+' .body');
            bdy.html($(this).html());
        });
    ...
    Htmle.prototype._createTools = function () {
		...
		if(scope.options.toolbarContainer)
        var htmlearea = this.$E.parents().find(scope.options.toolbarContainer);
		else
        var htmlearea = this.$E.parents().find('.htmle');
		...
		if(htmleStandardTools.children().length > 0)	
        ...
        if(htmleParagraphTools.children().length > 0)
        ...
        if(htmleParagraphTools.children().length > 0)
        htmleParagraphTools.append(this._createSeparate());
        if (this._includeControl('btn-orderedlist'))
		...
		if(htmleInsertTools.children().length > 0)	
        htmleInsertTools.append(this._createSeparate());

		    if(htmleStandardTools.children().length > 0)
            htmleTools.append(htmleStandardTools);
			
			if(htmleInsertTools.children().length > 0)
            htmleTools.append(htmleInsertTools);
			
			if(htmleSelectionTools.children().length > 0)
            htmleTools.append(htmleSelectionTools);
			
			if(htmleParagraphTools.children().length > 0)
            htmleTools.append(htmleParagraphTools);
			
			if(htmleFontTools.children().length > 0)
            htmleTools.append(htmleFontTools);
    ...
    Htmle.prototype._includeControl = function (name)
    {
        ...
        if (this.options.controlsIncluded)
        	{if($.inArray(name, this.options.controlsIncluded)!=-1)
				return true;
				else
				return false;
			}
		
       if (this.options.controlsExcluded && $.inArray(name, this.options.controlsExcluded) != -1)
			{
            return false;
		    }
    ...
    Htmle.prototype._applyCommand = function (arguments, source) {
        var bdy = this.$E.parents().find(this.options.textareaContainer+' .body');
        if (bdy.css('display') !== 'none') {
            bdy.focus();
    ...
    Htmle.prototype._switchMode = function () {
        var bdy = this.$E.parents().find(scope.options.textareaContainer+' .body');
        if (bdy.css('display') !== 'none') {
            bdy
            ...
            var cod = this.$E.parents().find(scope.options.textareaContainer+' .codebody');
            if (cod.length > 0) {
                bdy.html(cod.text());
                cod.remove();
            }
            ...
            var area = this.$E.parents().find(scope.options.textareaContainer+' .editview');
