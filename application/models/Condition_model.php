...
class Condition_model extends CI_Model {
	...
	public function getCondition($filters=array())
	{
			...
			array_push($WhereSubQueryArry,'(`condition_id` IS NOT NULL) ');			
			...
			{  
				...
				else
				{
			   	...
			   	foreach($valueFormated as $key2 => $value2)
			   	{
					...
					{
						array_push($miniWhereSubQueryArry,'`condition_id`=:condition_id'.$n ); 
						$n++;
					}elseif($key==='condition_name' || ($key==='condition_name'.$n) )
					{
						array_push($miniWhereSubQueryArry,'`condition_name`=:condition_name'.$n ); 
						$n++;
				...
				foreach($filters as $key => $value)
				{
					if(is_array($value))
					 ...
					 else
				 	{
				   	...
				   	{					   
						if($key==='condition_id' || ($key==='condition_id'.$n) ) 
						{$dbquery1->bindValue(":condition_id".$n,$value2); 	$n++;}
						
						if($key==='condition_name' || ($key==='condition_name'.$n) ) 
						{$dbquery1->bindValue(":condition_name".$n,$value2); $n++; }
